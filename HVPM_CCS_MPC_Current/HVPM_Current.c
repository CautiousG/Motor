...
_iq IqRef = _IQ(0.25);				// Iq reference (pu)
...
_iq  SpeedRef = _IQ(0.13333333333);           // For Closed Loop tests 0.13333333333  0.26666666667
#endif
...
// Initialize the PI1 module for speed
    ...
    pi1_spd.Umax = _IQ(2.5);       // 9.5              2.69
    pi1_spd.Umin = _IQ(-2.5);      // -9.5             -2.69
    pi1_spd.bd = _IQ(0.05);

// Initialize the PI1 module for Id
    pi1_id.Kp = _IQ(7.39);      // 4.0
    pi1_id.Ki = _IQ(0.0185);       // 0.7  0.0185
...
// Initialize the PI1 module for Iq
    ...
    pi1_iq.Ki = _IQ(0.0185);       // 0.7  0.0185
...
//    }
    ...
    static Uint32 i1 = 0;
    static Uint32 i2 = 0;
    static int status = 0;  //  0: RPM:0-400    Current:0.5
                            //  1: RPM:400      Current:0.5-1.0
                            //  2: RPM:400      Current:1   Exit
    if(lsw == 1 || lsw == 2)
    {
        if(status == 0)
        {
...
// ------------------------------------------------------------------------------
    ...
    else if(lsw == 1)
    {
        ...
        PMSM_MPC_CCS_Current(cmc.id, cmc.iq, cmc.we, cmc.idref, cmc.iqref, &vd, &vq, cmc.Ts, pm);
        cmc.vd = vd;
        cmc.vq = vq;
        pi1_id.Out = vd;
        pi1_iq.Out = vq;
...
//    }
    ...
    if(lsw == 1 || lsw == 2)
    {
        ...
        if( (status1 == 1) || (status1 == 2) || (ii1 >= (Delay - 6000)) )
        {
            testdata[0] = clarke1.As*10;
            testdata[1] = speed1.SpeedRpm;
            testdata[2] = cmc.id;
